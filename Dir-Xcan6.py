...
#
# DIR-XCAN5.PY
# This program is for finding hidden directories that are not directly linked on a website.
# It find HTTP response code 200 directories and outputs the URL to file.
    ...
    if results['c'] is not None:  ####### need to change imput to : not ; as it fuckes with life!
        ...
        if re.search(';', results['c']):
            ...
            for authcookies in cookielist:
                ...
                if re.search('[":_-/a-zA-Z0-9]', authcookies) and re.search('[=]',
                                                                            authcookies):  ##### Error here too, regex all fucked up
...
def GetURL(host, target):
    ...
    sys.stdout.write(
        "\r\x1b[K \033[31m%d \033[0mFound, \033[33m%d \033[0mForbidden, \033[32m%d \033[0mNotFound, \033[37m%d \033[0mOther, \033[37m%d \033[0mPercent Left" % (
            Found, Forbidden, NotFound, Other, LinesLeft * 100 / Lines))
    sys.stdout.flush()

    url = target + '/' + str(host.rstrip())
    try:
        if AuthOpt:
            response = requesocks.get(url, auth=(Auth_User, Auth_Pwd), headers=headers)
        elif ProxyOpt:
            response = requesocks.get(target + '/' + str(host.rstrip()), proxies=Proxies, headers=headers)
        elif AuthOpt and ProxyOpt:
            response = requesocks.get(target + '/' + str(host.rstrip()), proxies=Proxy_Addr, auth=(Auth_User, Auth_Pwd),
                                 headers=headers)
        elif CookiesOpt:
            response = requesocks.get(target + '/' + str(host.rstrip()), cookies=cookies, headers=headers)
        else:
            response = requesocks.get(target + '/' + str(host.rstrip()), headers=headers)
        ...
        if code == 401:
            ...
            LinesLeft -= 1
            outputfile.write(
                "<A HREF='" + target + "/" + host + "'>" + target + '/' + host + " - <STRONG>REQUIRES AUTHENTICATION</STRONG><br>\n")
            if results['V']:
                sys.stdout.write("\r\x1b[K\033[32m %s/%s\033[0m-REQUIRES AUTHENTICATION" % (
                    target, host))  # Doesnt print after value :S
                sys.stdout.flush()
            ...
            LinesLeft -= 1
            outputfile.write(
                "<A HREF='" + target + "/" + host + "'>" + target + '/' + host +
                " - <STRONG>FORBIDDEN</STRONG><br>\n")
            if results['V']:
                sys.stdout.write("\r\x1b[K\033[33m %s/%s\033[0m-FORBIDDEN" % (
                    target, host))  # Doesnt print after value :S
                sys.stdout.flush()
...
def main():
    threads = [ThreadUrl(queue) for i in range(ThreadNumber)]
